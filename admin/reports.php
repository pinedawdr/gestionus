...
// Definir variables para reportes
...
$endDate = isset($_GET['end_date']) ? sanitize($_GET['end_date']) : date('Y-m-d'); // Día actual
...
if (!empty($reportType)) {
    switch ($reportType) {
        case 'uploads_by_user':
            $reportTitle = 'Documentos Subidos por Usuario';
            
            // Consulta para documentos por usuario
            $sql = "SELECT u.name, COUNT(d.id) as total_docs 
                    FROM users u 
            ...
            if ($selectedUserId > 0) {
                ...
                // Solo incluir usuarios activos si no se seleccionó uno específico
                $whereConditions[] = "u.active = 1";
            }
            ...
            $sql .= " GROUP BY u.id, u.name ORDER BY total_docs DESC";
            
            $reportData = $db->fetchAll($sql, $params);
            
            // Preparar datos para gráfico
            foreach ($reportData as $item) {
                $chartLabels[] = $item['name'];
                $chartData[] = $item['total_docs'];
                $chartColors[] = 'rgba(54, 162, 235, 0.7)';
            }
            break;
            
        case 'uploads_by_type':
            $reportTitle = 'Documentos por Tipo';
            
            // Consulta para documentos por tipo
            $sql = "SELECT d.type, COUNT(d.id) as total 
                    FROM documents d";
            ...
            $sql .= " GROUP BY d.type ORDER BY total DESC";
            ...
            foreach ($reportData as $item) {
                ...
                switch ($item['type']) {
                    case 'backup':
                        ...
                        break;
                    case 'evidencia':
                        $typeLabel = 'Evidencias';
                        $color = 'rgba(75, 192, 192, 0.7)';
                        break;
                    case 'reporte_cnv':
                        $typeLabel = 'Reportes CNV';
        ...
        case 'uploads_by_date':
            $reportTitle = 'Documentos por Fecha';
            
            // Consulta para documentos por fecha
            $sql = "SELECT DATE(d.created_at) as upload_date, COUNT(d.id) as total 
                    FROM documents d";
            ...
            $sql .= " GROUP BY DATE(d.created_at) ORDER BY upload_date ASC";
        ...
        case 'missing_uploads':
            ...
            if ($selectedUserId > 0) {
                $sql .= " AND id = ?";
                $activeUsers = $db->fetchAll($sql, [$selectedUserId]);
            } else {
                $activeUsers = $db->fetchAll($sql);
            }
            
            // Calcular fechas para las verificaciones
            $checkDate = new DateTime();
            if (!empty($endDate)) {
                $checkDate = new DateTime($endDate);
            }
            
            $reportData = [];
            
            // Verificar cada tipo de documento para cada usuario
            foreach ($activeUsers as $activeUser) {
                $missingDocs = [];
                
                // Verificar backups (viernes)
                $lastFriday = clone $checkDate;
                while ($lastFriday->format('N') != 5) { // 5 = viernes
                    $lastFriday->modify('-1 day');
                }
                
                $sql = "SELECT COUNT(*) as count FROM documents 
                        WHERE user_id = ? AND type = 'backup' 
                        AND DATE(created_at) = ?";
                $result = $db->fetchOne($sql, [$activeUser['id'], $lastFriday->format('Y-m-d')]);
                
                if ($result['count'] == 0) {
                    $missingDocs[] = [
                        'type' => 'backup',
                        'due_date' => $lastFriday->format('d/m/Y'),
                        'status' => 'Pendiente'
                    ];
                }
                
                // Verificar evidencias (viernes)
                $sql = "SELECT COUNT(*) as count FROM documents 
                ...
                // Verificar reportes CNV (martes)
                $lastTuesday = clone $checkDate;
                while ($lastTuesday->format('N') != 2) { // 2 = martes
                    $lastTuesday->modify('-1 day');
                }
                
                $sql = "SELECT COUNT(*) as count FROM documents 
                        WHERE user_id = ? AND type = 'reporte_cnv' 
                        AND DATE(created_at) = ?";
                $result = $db->fetchOne($sql, [$activeUser['id'], $lastTuesday->format('Y-m-d')]);
                
                if ($result['count'] == 0) {
                    $missingDocs[] = [
                        'type' => 'reporte_cnv',
                        'due_date' => $lastTuesday->format('d/m/Y'),
                        'status' => 'Pendiente'
                    ];
                }
...
<head>
    ...
    <title>Reportes - GestiónUS</title>
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
                            ...
                            <a href="reports.php" class="border-b-2 border-white px-1 pt-1 text-sm font-medium">Reportes</a>
...
Le recordamos que tiene un documento pendiente por subir en el sistema GestiónUS. Por favor, suba el documento requerido a la brevedad posible.
...
Le recordamos que tiene pendiente la subida del ${docTypeName} en el sistema GestiónUS. Por favor, suba el documento requerido a la brevedad posible.

Saludos cordiales,
Administración GestiónUS`;
            }
