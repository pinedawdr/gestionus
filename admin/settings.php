...
// Procesar formulario de configuración de correo
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'email_config') {
    $emailjsUserId = sanitize($_POST['emailjs_user_id']);
    $emailjsServiceId = sanitize($_POST['emailjs_service_id']);
    $emailjsTemplateId = sanitize($_POST['emailjs_template_id']);
    
    // Validar datos
    if (empty($emailjsUserId) || empty($emailjsServiceId) || empty($emailjsTemplateId)) {
        $errorMessage = 'Todos los campos son obligatorios';
    } else {
        // Actualizar archivo config.php
        $configFile = file_get_contents(BASE_PATH . '/includes/config.php');
        
        // Reemplazar valores
        $configFile = preg_replace('/define\(\'EMAILJS_USER_ID\',\s*\'.*?\'\);/', "define('EMAILJS_USER_ID', '$emailjsUserId');", $configFile);
        $configFile = preg_replace('/define\(\'EMAILJS_SERVICE_ID\',\s*\'.*?\'\);/', "define('EMAILJS_SERVICE_ID', '$emailjsServiceId');", $configFile);
        $configFile = preg_replace('/define\(\'EMAILJS_TEMPLATE_ID\',\s*\'.*?\'\);/', "define('EMAILJS_TEMPLATE_ID', '$emailjsTemplateId');", $configFile);
        
// Guardar archivo
if (file_put_contents(BASE_PATH . '/includes/config.php', $configFile)) {
    $successMessage = 'Configuración de correo actualizada correctamente';
} else {
    $errorMessage = 'Error al guardar la configuración. Verifica los permisos del archivo.';
}
}
}

// Procesar formulario de configuración de Firebase
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'firebase_config') {
$firebaseApiKey = sanitize($_POST['firebase_api_key']);

// Validar datos
if (empty($firebaseApiKey)) {
$errorMessage = 'La API Key es obligatoria';
} else {
// Actualizar archivo config.php
$configFile = file_get_contents(BASE_PATH . '/includes/config.php');

// Reemplazar valores
$configFile = preg_replace('/define\(\'FIREBASE_API_KEY\',\s*\'.*?\'\);/', "define('FIREBASE_API_KEY', '$firebaseApiKey');", $configFile);

// Guardar archivo
if (file_put_contents(BASE_PATH . '/includes/config.php', $configFile)) {
    // Actualizar archivo firebase-config.js
    $firebaseConfigFile = "// Configuración de Firebase
const firebaseConfig = {
apiKey: \"$firebaseApiKey\", // Reemplazar con tu API Key
authDomain: \"tu-proyecto.firebaseapp.com\", // Reemplazar con tu Auth Domain
projectId: \"tu-proyecto\", // Reemplazar con tu Project ID
storageBucket: \"tu-proyecto.appspot.com\", // Reemplazar con tu Storage Bucket
messagingSenderId: \"tu-messaging-sender-id\", // Reemplazar con tu Messaging Sender ID
appId: \"tu-app-id\" // Reemplazar con tu App ID
};

// Inicializar Firebase
firebase.initializeApp(firebaseConfig);";
    
    if (file_put_contents(BASE_PATH . '/firebase-config.js', $firebaseConfigFile)) {
        $successMessage = 'Configuración de Firebase actualizada correctamente';
    } else {
        $errorMessage = 'Error al guardar la configuración de Firebase. Verifica los permisos del archivo.';
    }
} else {
    $errorMessage = 'Error al guardar la configuración. Verifica los permisos del archivo.';
}
...
// Procesar formulario de configuración de base de datos
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'db_config') {
$dbHost = sanitize($_POST['db_host']);
$dbUser = sanitize($_POST['db_user']);
$dbPass = sanitize($_POST['db_pass']);
$dbName = sanitize($_POST['db_name']);

// Validar datos
if (empty($dbHost) || empty($dbUser) || empty($dbName)) {
$errorMessage = 'Host, Usuario y Nombre de la base de datos son obligatorios';
} else {
// Actualizar archivo config.php
$configFile = file_get_contents(BASE_PATH . '/includes/config.php');

// Reemplazar valores
$configFile = preg_replace('/define\(\'DB_HOST\',\s*\'.*?\'\);/', "define('DB_HOST', '$dbHost');", $configFile);
$configFile = preg_replace('/define\(\'DB_USER\',\s*\'.*?\'\);/', "define('DB_USER', '$dbUser');", $configFile);
$configFile = preg_replace('/define\(\'DB_PASS\',\s*\'.*?\'\);/', "define('DB_PASS', '$dbPass');", $configFile);
$configFile = preg_replace('/define\(\'DB_NAME\',\s*\'.*?\'\);/', "define('DB_NAME', '$dbName');", $configFile);

// Guardar archivo
if (file_put_contents(BASE_PATH . '/includes/config.php', $configFile)) {
    $successMessage = 'Configuración de base de datos actualizada correctamente';
} else {
    $errorMessage = 'Error al guardar la configuración. Verifica los permisos del archivo.';
}
}
...
// Procesar creación de la base de datos
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_db') {
try {
// Conectar a la base de datos sin seleccionar una base de datos específica
$pdo = new PDO(
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `users` (
    ...
    `punto_digitacion` varchar(255) NOT NULL,
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `documents` (
    ...
    `type` enum('backup','evidencia','reporte_cnv','otro') NOT NULL,
...
if ($result['count'] == 0) {
    ...
    $successMessage = 'Base de datos y tablas creadas correctamente. Se han creado las carpetas necesarias para los archivos.';
} else {
    $successMessage = 'Base de datos y tablas verificadas correctamente.';
}
} catch (PDOException $e) {
$errorMessage = 'Error al crear la base de datos: ' . $e->getMessage();
}
}
?>
<!DOCTYPE html>
...
<title>Configuración - GestiónUS</title>
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
                    ...
                    <a href="settings.php" class="border-b-2 border-white px-1 pt-1 text-sm font-medium">Configuración</a>
                </div>
            </div>
